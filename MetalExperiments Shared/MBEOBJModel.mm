...
@interface MBEOBJModel ()
{
    ...
    std::vector<WLVertex> groupVertices;
    std::vector<WLInt16> groupIndices;
    std::map<FaceVertex, WLInt16> vertexToGroupIndexMap;
}
...
- (void)endCurrentGroup
{
    ...
    NSData *vertexData = [NSData dataWithBytes:groupVertices.data() length:sizeof(WLVertex) * groupVertices.size()];
    self.currentGroup.vertexData = vertexData;

    NSData *indexData = [NSData dataWithBytes:groupIndices.data() length:sizeof(WLInt16) * groupIndices.size()];
